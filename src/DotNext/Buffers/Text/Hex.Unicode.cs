...
public static partial class Hex
{
    ...
    public static int EncodeToUtf16(scoped ReadOnlySpan<byte> bytes, scoped Span<char> output, bool lowercased = false)
    {
        ...
        if (Ssse3.IsSupported)
        {
            ...
            if (Avx2.IsSupported && offset >= sizeof(ulong))
            {
                ...
                do
                {
                    ...
                    Vector256.StoreUnsafe(result, ref As<char, byte>(ref charPtr));
                ...
                // load 8 bytes to the vector
                [MethodImpl(MethodImplOptions.AggressiveInlining)]
                static Vector256<byte> Fetch(ref byte bytePtr)
                {
                    var tmp = ReadUnaligned<Tuple64>(ref bytePtr);
            ...
            if (offset >= sizeof(uint))
            {
                ...
                do
                {
                    ...
                    var highNibbles = Vector128.ShiftRightLogical(lowNibbles.AsUInt32(), 4).AsByte();
                    ...
                    // save vector back to memory block
                    Vector128.StoreUnsafe(result, ref As<char, byte>(ref charPtr));
