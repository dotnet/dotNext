...
public static partial class DelegateHelpers
{
    ...
    private static TDelegate ChangeType<TDelegate, TRewriter>(this Delegate d, TRewriter rewriter) where TDelegate : Delegate
        where TRewriter : struct, ISupplier<Delegate, object?>
    {
        if (d.GetInvocationList() is not [_, ..] list)
        {
