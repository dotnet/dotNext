        ...
        public static string ReadString(this Stream stream, int length, DecodingContext context, byte[] buffer)
        {
            ...
            try
            {
                do
                {
                    ...
                    var charsRead = decoder.GetChars(buffer, 0, n, charBuffer.UnsafeGetArray(), 0);
                    if (currentPos == 0 && n == length)
                        return new string(charBuffer.UnsafeGetArray(), 0, charsRead);
                    if (result.IsEmpty)
                ...
                return new string(result.UnsafeGetArray(), 0, resultOffset);
            }
            finally
            {
                charBuffer.Dispose();
