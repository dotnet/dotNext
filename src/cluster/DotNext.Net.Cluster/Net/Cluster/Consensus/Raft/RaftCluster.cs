...
namespace DotNext.Net.Cluster.Consensus.Raft
{
        ...
        where TMember : class, IRaftClusterMember, IDisposable
    {
        ...
        protected async Task ChangeMembersAsync<T>(MemberCollectionMutator<T> mutator, T arg, CancellationToken token)
        {
            ...
            var transitionLock = await transitionSync.TryAcquireAsync(token).SuppressDisposedStateAndCancellation().ConfigureAwait(false);
            try
            {
                if (transitionLock)
        ...
        async void IRaftStateMachine.MoveToFollowerState(bool randomizeTimeout, long? newTerm)
        {
            ...
            using var lockHolder = await transitionSync.TryAcquireAsync(Token).SuppressDisposedStateAndCancellation().ConfigureAwait(false);
            if (lockHolder)
            {
                if (randomizeTimeout)
                    electionTimeout = electionTimeoutProvider.RandomTimeout();
