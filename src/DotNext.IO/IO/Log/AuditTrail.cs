...
namespace DotNext.IO.Log
{
    ...
    /// <summary>
    /// Represents general purpose persistent audit trail.
    /// </summary>
    /// <remarks>
    ...
    /// </remarks>
    public class PersistentState : Disposable, IAuditTrail<ILogEntry>
    {
        internal readonly struct LogEntryMetadata
        {
            internal readonly long Timestamp, Length, Offset;

            internal LogEntryMetadata(ILogEntry entry, long offset, long length)
            {
            ...
            private LogEntryMetadata(in LogEntryMetadata metadata, long offsetDelta)
            {
                Timestamp = metadata.Timestamp;
                Length = metadata.Length - offsetDelta;
                Offset = metadata.Offset + offsetDelta;
            }

            internal LogEntryMetadata AddOffset(long offset) => new LogEntryMetadata(this, offset);

            internal unsafe static int Size => sizeof(LogEntryMetadata);
        }
        ...
        internal readonly struct SnapshotMetadata
        {
            ...
            internal SnapshotMetadata(ILogEntry snapshot, long index, long length)
            {
                Index = index;
                RecordMetadata = new LogEntryMetadata(snapshot, Size, length);
            }

            internal unsafe static int Size => sizeof(SnapshotMetadata);
        }
        ...
        protected readonly struct LogEntry : ILogEntry
        {
            ...
            private LogEntry(StreamSegment cachedContent, Memory<byte> sharedBuffer, in LogEntryMetadata metadata, long? snapshotIndex)
            {
                timeStamp = metadata.Timestamp;
                content = cachedContent;
                buffer = sharedBuffer;
                SnapshotIndex = null;
                content.Adjust(metadata.Offset, metadata.Length);
                SnapshotIndex = snapshotIndex;
            }

            internal LogEntry(StreamSegment cachedContent, Memory<byte> sharedBuffer, in LogEntryMetadata metadata)
                : this(cachedContent, sharedBuffer, metadata, null)
            {
            }

            internal LogEntry(StreamSegment cachedContent, Memory<byte> sharedBuffer, in SnapshotMetadata metadata)
                : this(cachedContent, sharedBuffer, in metadata.RecordMetadata, metadata.Index)
            {
            }
            ...
            /// </summary>
            public long Length => content.Length - content.Position;
            ...
            /// <summary>
            /// Deserializes value of blittable type from the log entry.
            /// </summary>
            /// <typeparam name="T">The blittable type to deserialize.</typeparam>
            /// <returns>Deserialized value of blittable type.</returns>
            public T Read<T>() where T : unmanaged => content.Read<T>();
            ...
            /// <summary>
            /// Deserializes value of blittable type from the log entry asynchronously.
            /// </summary>
            /// <typeparam name="T">The blittable type to deserialize.</typeparam>
            /// <param name="token">The token that can be used to cancel the operation.</param>
            /// <returns>Deserialized value of blittable type.</returns>
            public ValueTask<T> ReadAsync<T>(CancellationToken token = default) where T : unmanaged => content.ReadAsync<T>(token);
            ...
            public async ValueTask CopyToAsync(Stream output, CancellationToken token) => await content.CopyToAsync(output, buffer, token).ConfigureAwait(false);
            ...
            /// <param name="token">The token that can be used to cancel asynchronous operation.</param>
            /// <param name="output">The output stream receiving object content.</param>
            public void CopyTo(Stream output, CancellationToken token) => content.CopyTo(output, buffer.Span, token);
            ...
            public async ValueTask CopyToAsync(PipeWriter output, CancellationToken token) => await content.ReadAsync(output, token: token).ConfigureAwait(false);
            ...
            /// </summary>
            public DateTimeOffset Timestamp => new DateTimeOffset(timeStamp, TimeSpan.Zero);
        }
        ...
        /// </summary>
        protected abstract class SnapshotBuilder : Disposable, ILogEntry
        {
