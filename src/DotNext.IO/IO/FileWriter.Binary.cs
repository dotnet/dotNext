...
public partial class FileWriter : IAsyncBinaryWriter
{
    ...
    {
        if (FreeCapacity < SevenBitEncodedInteger<uint>.MaxSizeInBytes)
            await FlushAsync(token).ConfigureAwait(false);
        ...
        where T : notnull, IUtf8SpanFormattable
    {
        var expectedLengthSize = lengthFormat switch
        {
            ...
            LengthFormat.Compressed => SevenBitEncodedInteger<uint>.MaxSizeInBytes,
        ...
        where T : notnull, IUtf8SpanFormattable
    {
        ...
        {
            const int maxBufferSize = int.MaxValue / 2;
            for (var bufferSize = MaxBufferSize + SevenBitEncodedInteger<uint>.MaxSizeInBytes; ; bufferSize = bufferSize <= maxBufferSize ? bufferSize << 1 : throw new InsufficientMemoryException())
            {
