...
public partial class FileWriter : IAsyncBinaryWriter
{
    ...
    {
        if (FreeCapacity < ULeb128<uint>.MaxSizeInBytes)
            await FlushAsync(token).ConfigureAwait(false);
        ...
        where T : notnull, IUtf8SpanFormattable
    {
        var expectedLengthSize = lengthFormat switch
        {
            ...
            LengthFormat.Compressed => ULeb128<uint>.MaxSizeInBytes,
        ...
        where T : notnull, IUtf8SpanFormattable
    {
        ...
        {
            const int maxBufferSize = int.MaxValue / 2;
            for (var bufferSize = MaxBufferSize + ULeb128<uint>.MaxSizeInBytes; ; bufferSize = bufferSize <= maxBufferSize ? bufferSize << 1 : throw new InsufficientMemoryException())
            {
