...
{
    /// <summary>
    /// Adds static factory methods to <see cref="TextWriter"/> type.
    /// </summary>
    /// <param name="receiver">The stream to write into.</param>
    extension(TextWriter receiver)
    {
        /// <summary>
        ...
        public static TextWriter Create<TWriter>(TWriter writer, IFormatProvider? provider = null, Action<TWriter>? flush = null, Func<TWriter, CancellationToken, Task>? flushAsync = null)
        ...
        public static TextWriter Create<TWriter>(TWriter writer, Encoding encoding, IFormatProvider? provider = null, Action<TWriter>? flush = null, Func<TWriter, CancellationToken, Task>? flushAsync = null)
        ...
        public async ValueTask WriteAsync(ReadOnlySequence<char> chars, CancellationToken token = default)
        {
            foreach (var segment in chars)
                await receiver.WriteAsync(segment, token).ConfigureAwait(false);
        }
        ...
        private async ValueTask WriteAsync(MemoryOwner<char> buffer, CancellationToken token)
        {
            try
            {
                await receiver.WriteAsync(buffer.Memory, token).ConfigureAwait(false);
            }
            finally
            {
                buffer.Dispose();
        ...
        /// <param name="token">The token that can be used to cancel the operation.</param>
        /// <returns>A task that represents the asynchronous write operation.</returns>
        public ValueTask WriteAsync(MemoryAllocator<char>? allocator, [InterpolatedStringHandlerArgument(nameof(allocator))] ref PoolingInterpolatedStringHandler handler, CancellationToken token = default)
            => WriteAsync(receiver, allocator, null, ref handler, token);
        ...
        /// <param name="token">The token that can be used to cancel the operation.</param>
        /// <returns>A task that represents the asynchronous write operation.</returns>
        public ValueTask WriteLineAsync(MemoryAllocator<char>? allocator, IFormatProvider? provider, [InterpolatedStringHandlerArgument(nameof(allocator), nameof(provider))] ref PoolingInterpolatedStringHandler handler, CancellationToken token = default)
        {
            handler.AppendLiteral(Environment.NewLine);
            return WriteAsync(receiver, allocator, provider, ref handler, token);
        }
        ...
        /// <param name="token">The token that can be used to cancel the operation.</param>
        /// <returns>A task that represents the asynchronous write operation.</returns>
        public ValueTask WriteLineAsync(MemoryAllocator<char>? allocator, [InterpolatedStringHandlerArgument(nameof(allocator))] ref PoolingInterpolatedStringHandler handler, CancellationToken token = default)
            => WriteLineAsync(receiver, allocator, null, ref handler, token);
        ...
        /// <param name="allocator">The allocator for the buffer used by string interpolation handler.</param>
        /// <param name="handler">The interpolated string handler.</param>
        public void Write(MemoryAllocator<char>? allocator, [InterpolatedStringHandlerArgument(nameof(allocator))] ref PoolingInterpolatedStringHandler handler)
            => Write(receiver, allocator, null, ref handler);
        ...
        /// <param name="allocator">The allocator for the buffer used by string interpolation handler.</param>
        /// <param name="handler">The interpolated string handler.</param>
        public void WriteLine(MemoryAllocator<char>? allocator, [InterpolatedStringHandlerArgument(nameof(allocator))] ref PoolingInterpolatedStringHandler handler)
            => WriteLine(receiver, allocator, null, ref handler);
    }

    /// <summary>
    /// Adds static factory methods to <see cref="TextReader"/> type.
    /// </summary>
    /// <param name="receiver">Readable stream.</param>
    extension(TextReader receiver)
    {
        /// <summary>
        ...
        public static TextReader Create(ReadOnlySequence<char> sequence)
            => sequence.IsEmpty ? TextReader.Null : new CharBufferReader(sequence);
        ...
        public static TextReader Create(ReadOnlySequence<byte> sequence, Encoding encoding, int bufferSize = 1024,
            MemoryAllocator<char>? allocator = null)
            => sequence.IsEmpty ? TextReader.Null : new DecodingTextReader(sequence, encoding, bufferSize, allocator);
        ...
        public async IAsyncEnumerable<ReadOnlyMemory<char>> ReadAllAsync(int bufferSize, MemoryAllocator<char>? allocator = null, [EnumeratorCancellation] CancellationToken token = default)
        {
            ArgumentOutOfRangeException.ThrowIfNegativeOrZero(bufferSize);

            using var buffer = allocator.DefaultIfNull.AllocateAtLeast(bufferSize);

            for (int count; (count = await receiver.ReadAsync(buffer.Memory, token).ConfigureAwait(false)) > 0;)
