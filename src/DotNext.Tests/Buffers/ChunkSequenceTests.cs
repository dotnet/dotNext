using System.Buffers;
using System.Text;

namespace DotNext.Buffers;

public sealed class ChunkSequenceTests : Test
{
    [Fact]
    public static void Concatenation2()
    {
        ReadOnlyMemory<byte> block1 = default, block2 = default;
        Equal([], new[] { block1, block2 }.Concat().ToArray());

        block1 = new byte[] { 1, 2 };
        Equal([1, 2], new List<ReadOnlyMemory<byte>> { block1, block2 }.Concat().ToArray());

        block2 = block1;
        block1 = default;
        Equal([1, 2], ToEnumerable(block1, block2).Concat().ToArray());

        block1 = new byte[] { 3, 4 };
        Equal([1, 2, 3, 4], new[] { block2, block1 }.Concat().ToArray());

        static IEnumerable<ReadOnlyMemory<byte>> ToEnumerable(ReadOnlyMemory<byte> block1, ReadOnlyMemory<byte> block2)
        {
            yield return block1;
            yield return block2;
        }
    }

    [Fact]
    public static void Concatenation3()
    {
        Equal([], ReadOnlyMemory<byte>.Concat().ToArray());
        
        ReadOnlyMemory<byte> block1 = new byte[] { 1, 2 };
        Equal(block1.Span, ReadOnlyMemory<byte>.Concat(block1).ToArray());
        
        ReadOnlyMemory<byte> block2 = new byte[] { 3, 4 };
        Equal([1, 2, 3, 4], ReadOnlyMemory<byte>.Concat(block1, block2).ToArray());
    }

    [Fact]
    public static void StringConcatenation1()
    {
        string block1 = string.Empty, block2 = null;
        Equal(string.Empty, new[] { block1, block2 }.Concat().ToString());

        block1 = "Hello";
        Equal(block1, new List<string> { block1, block2 }.Concat().ToString());

        block2 = block1;
        block1 = default;
        Equal(block2, ToEnumerable(block1, block2).Concat().ToString());

        block1 = "Hello, ";
        block2 = "world!";
        Equal(block1 + block2, new[] { block1, block2 }.Concat().ToString());

        static IEnumerable<string> ToEnumerable(string block1, string block2)
        {
            yield return block1;
            yield return block2;
        }
    }
    
    [Fact]
    public static void StringConcatenation2()
    {
        const string block1 = "Hello";
        Equal(block1, ReadOnlyMemory<char>.Concat(block1.AsMemory()).ToString());

        const string block2 = ", world!";
        Equal(block1 + block2, ReadOnlyMemory<char>.Concat(block1.AsMemory(), block2.AsMemory()).ToString());
    }

    [Fact]
    public static void CopyFromSequence()
    {
        var sequence = new ReadOnlySequence<byte>(new byte[] { 1, 2, 3 }.AsMemory());
        Span<byte> dest = new byte[3];
        var writtenCount = sequence >>> dest;
        Equal(3, writtenCount);
        Equal(sequence.ToArray(), dest.ToArray());

        sequence = ToReadOnlySequence<byte>(RandomBytes(64), 16);
        dest = new byte[64];
        writtenCount = sequence >>> dest;
        Equal(64, writtenCount);
        Equal(sequence.ToArray(), dest.ToArray());

        dest = new byte[100];
        writtenCount = sequence >>> dest;
        Equal(64, writtenCount);
        Equal(sequence.ToArray(), dest.Slice(0, 64).ToArray());

        dest = new byte[10];
        writtenCount = sequence >>> dest;
        Equal(10, writtenCount);
        Equal(sequence.Slice(0, 10).ToArray(), dest.ToArray());
    }

    [Theory]
    [InlineData(0)]
    [InlineData(16)]
    [InlineData(128)]
    [InlineData(124)]
    public static void StringBuilderToSequence(int stringLength)
    {
        var str = Random.Shared.GetString(Alphabet, stringLength);

        var builder = new StringBuilder();
        for (var i = 0; i < 3; i++)
        {
            builder.Append(str);
        }

        Equal(builder.ToString(), builder.ToSequence().ToString());
    }

    [Fact]
    public static void CopyFromSequenceAndAdjustPosition()
    {
        var destination = new byte[10];
        ReadOnlySequence<byte> source = new(RandomBytes(16));

        // source > destination, single segment
        var bytesWritten = source.CopyTo(destination, out SequencePosition consumed);
        Equal(bytesWritten, destination.Length);
        Equal(destination, source.Slice(0, consumed).ToArray());

        // source > destination, multisegment, segment < destination
        Array.Clear(destination);
        source = ToReadOnlySequence<byte>(RandomBytes(16), 3);
        bytesWritten = source.CopyTo(destination, out consumed);
        Equal(destination.Length, bytesWritten);
        Equal(destination, source.Slice(0, consumed).ToArray());

        // source > destination, multisegment, segment > destination
        Array.Clear(destination);
        source = ToReadOnlySequence<byte>(RandomBytes(22), 11);
        bytesWritten = source.CopyTo(destination, out consumed);
        Equal(destination.Length, bytesWritten);
        Equal(destination, source.Slice(0, consumed).ToArray());

        // source < destination, multisegment, segment < destination
        Array.Clear(destination);
        source = ToReadOnlySequence<byte>(RandomBytes(6), 3);
        bytesWritten = source.CopyTo(destination, out consumed);
        Equal(source.Length, bytesWritten);
        Equal(source.End, consumed);
        Equal(destination.AsSpan(0, (int)source.Length), source.Slice(0, consumed).ToArray().AsSpan());
        
        // source < destination, single segment
        Array.Clear(destination);
        source = new(RandomBytes(3));
        bytesWritten = source.CopyTo(destination, out consumed);
        Equal(source.Length, bytesWritten);
        Equal(source.End, consumed);
        Equal(destination.AsSpan(0, (int)source.Length), source.Slice(0, consumed).ToArray().AsSpan());
    }
    
    [Theory]
    [MemberData(nameof(EqualityComparisonData))]
    public static void CompareSequenceEquality(ReadOnlySequence<byte> sequence1, ReadOnlySequence<byte> sequence2, int comparison)
    {
        Equal(comparison is 0, sequence1.SequenceEqual(sequence2));
    }

    [Theory]
    [MemberData(nameof(EqualityComparisonData))]
    public static void CompareSequence(ReadOnlySequence<byte> sequence1, ReadOnlySequence<byte> sequence2, int comparison)
    {
        Equal(comparison, sequence1.SequenceCompareTo(sequence2));
    }

    public static TheoryData<ReadOnlySequence<byte>, ReadOnlySequence<byte>, int> EqualityComparisonData => new()
    {
        // positive cases
        {
            ReadOnlySequence<byte>.Empty,
            ReadOnlySequence<byte>.Empty,
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            0
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 3, 4 }, new byte[] { 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 3, 4 }, new byte[] { 5, 6 }),
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 3, 4, 5 }, new byte[] { 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1 }, new byte[] { 2, 3 }, new byte[] { 4, 5, 6 }),
            0
        },
        // negative cases
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 7 }, new byte[] { 4, 5, 6 }),
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            4
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 7 }, new byte[] { 4, 5, 6 }),
            -4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 7 }, new byte[] { 4, 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 7, 4 }, new byte[] { 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 7, 4 }, new byte[] { 5, 6 }),
            -4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 7, 4, 5 }, new byte[] { 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1 }, new byte[] { 2, 3 }, new byte[] { 4, 5, 6 }),
            4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2 }, new byte[] { 3, 4, 5 }, new byte[] { 6 }),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1 }, new byte[] { 2, 7 }, new byte[] { 4, 5, 6 }),
            -4
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            new ReadOnlySequence<byte>([1, 2, 3]),
            1
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3]),
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            -1
        },
    };

    [Theory]
    [MemberData(nameof(SpanComparisonData))]
    public static void CompareSpanEquality(ReadOnlySequence<byte> sequence, ReadOnlyMemory<byte> memory, int comparison)
    {
        Equal(comparison is 0, sequence.SequenceEqual(memory.Span));
    }

    [Theory]
    [MemberData(nameof(SpanComparisonData))]
    public static void CompareSpan(ReadOnlySequence<byte> sequence, ReadOnlyMemory<byte> memory, int comparison)
    {
        Equal(comparison, sequence.SequenceCompareTo(memory.Span));
    }

    public static TheoryData<ReadOnlySequence<byte>, ReadOnlyMemory<byte>, int> SpanComparisonData => new()
    {
        // positive cases
        {
            ReadOnlySequence<byte>.Empty,
            ReadOnlyMemory<byte>.Empty,
            0
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            new byte[] { 1, 2, 3, 4, 5, 6 },
            0
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            new byte[] { 1, 2, 3, 4, 5, 6 },
            0
        },
        // negative cases
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            ReadOnlyMemory<byte>.Empty,
            1
        },
        {
            ReadOnlySequence<byte>.Empty,
            new byte[] { 1, 2, 3, 4, 5, 6 },
            -1
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 6]),
            new byte[] { 1, 2, 3, 4, 5, 7 },
            -1
        },
        {
            new ReadOnlySequence<byte>([1, 2, 3, 4, 5, 7]),
            new byte[] { 1, 2, 3, 4, 5, 6 },
            1
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 7 }),
            new byte[] { 1, 2, 3, 4, 5, 6 },
            1
        },
        {
            ReadOnlyMemory<byte>.Concat(new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }),
            new byte[] { 1, 2, 3, 4, 5, 7 },
            -1
        },
    };
}