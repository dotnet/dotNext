...
public sealed class DiskBasedStateMachineTests : Test
{
    ...
    public static async Task EmptyLogEntry()
    {
        ...
        Func<IReadOnlyList<IRaftLogEntry>, long?, CancellationToken, ValueTask<Missing>> checker = static (entries, _, _) =>
        {
    ...
    public static async Task QueryAppendEntries(long partitionSize, bool caching, int concurrentReads)
    {
        ...
        IPersistentState state = new SimpleStateMachine(dir, new() { MaxConcurrentReads = concurrentReads, InitialPartitionSize = partitionSize, UseCaching = caching });
        try
        {
            // entry 1
            ...
            checker = async (entries, snapshotIndex, token) =>
            {
                ...
                Equal(entry1.Content.ToArray(), await entries[1].ToByteArrayAsync(token: token));
            ...
            checker = async (entries, snapshotIndex, token) =>
            {
                ...
                Equal(entry2.Content.ToArray(), await entries[0].ToByteArrayAsync(token: token));
                return Missing.Value;
            };
    ...
    public static async Task PartitionOverflow(bool useCaching)
    {
        ...
        Func<IReadOnlyList<IRaftLogEntry>, long?, CancellationToken, ValueTask<Missing>> checker;
        var dir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
        IPersistentState state = new SimpleStateMachine(dir, new() { UseCaching = useCaching, InitialPartitionSize = 1024 * 1024 });
        try
        {
            checker = (entries, snapshotIndex, _) =>
            {
                Null(snapshotIndex);
                Equal(1L, entries.Count);
                Equal(0L, entries[0].Term);
            ...
            checker = async (entries, snapshotIndex, token) =>
            {
                ...
                Equal(entry2.Content.ToArray(), await entries[2].ToByteArrayAsync(token: token));
                Equal(entry2.Timestamp, entries[2].Timestamp);
                Equal(44L, entries[3].Term);
                Equal(entry3.Content.ToArray(), await entries[3].ToByteArrayAsync(token: token));
                Equal(entry3.Timestamp, entries[3].Timestamp);
                Equal(45L, entries[4].Term);
                Equal(entry4.Content.ToArray(), await entries[4].ToByteArrayAsync(token: token));
                Equal(entry4.Timestamp, entries[4].Timestamp);
                Equal(46L, entries[5].Term);
                Equal(entry5.Content.ToArray(), await entries[5].ToByteArrayAsync(token: token));
            ...
            checker = async (entries, snapshotIndex, token) =>
            {
                ...
                Equal(entry4.Content.ToArray(), await entries[0].ToByteArrayAsync(token: token));

                Equal(entry5.Term, entries[1].Term);
                Equal(entry5.Content.ToArray(), await entries[1].ToByteArrayAsync(token: token));
                return null;
            };
