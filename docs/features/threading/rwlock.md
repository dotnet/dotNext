...
Behavior of `UpgradeToWriteLockAsync` and `DowngradeFromWriteLock` methods are the same as in [ReaderWriterLock](https://docs.microsoft.com/en-us/dotnet/api/system.threading.readerwriterlock):
1. `UpgradeToWriteLockAsync` releases the reader lock and tries to acquire the writer lock. If writer lock is not available for acquisition, the caller goes to the end of the wait queue
2. `UpgradeToWriteLockAsync` and `TryUpgradeToWriteLock` always release the reader lock even if the writer lock cannot be acquired immediately, or if the method throws
