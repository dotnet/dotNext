...
The feature is based on strongly typed reflection so read [this](reflection/fast.md) document first. Entry point to discover class members is [Type&lt;T&gt;](xref:DotNext.Reflection.Type`1). Reflected members are cached to speed-up performance and typed by specific delegate type. The delegate describes signature of the reflected member. All types of members are supported: constructors, fields, methods, event, properties, indexer properties.
...
[ConceptAttribute](xref:DotNext.Runtime.CompilerServices.ConceptAttribute) is required attribute that should be applied to the concept type definition.
...
`Require` method is used to obtain instance member from the type specified as actual generic argument for type `Type<T>`. Static member can be obtained using `RequireStatic` family of methods. `Require` and `RequireStatic` are useful in the context of concept type declaration. If target type _T_ doesn't have required member then these methods will throw one of [ConstraintViolationException](xref:DotNext.Reflection.ConstraintViolationException) ancestors:
* [MissingConstructorException](xref:DotNext.Reflection.MissingConstructorException) if required constructor doesn't exist
* [MissingMethodException](xref:DotNext.Reflection.MissingMethodException) if required method or property getter/setter doesn't exist
* [MissingFieldException](xref:DotNext.Reflection.MissingFieldException) if required field doesn't exist
* [MissingEventException](xref:DotNext.Reflection.MissingEventException) if required event doesn't exist
...
# Applying Concept
When concept type is declared, it can be used as a constraint for generic type parameter of class or method. [Concept](xref:DotNext.Runtime.CompilerServices.Concept) class allows to apply concept type to the generic parameter and verify constraints, thus, _fail fast_ if actual generic argument doesn't meet to the them. [ConstraintAttribute](xref:DotNext.Runtime.CompilerServices.ConstraintAttribute) indicating that the specified generic parameter is constrained with one or more concept types.
```csharp
using DotNext.Runtime.CompilerServices;
using System;
...
* Use special delegates provided by DotNext Reflection library:
    * [Function&lt;TArgs, TResult&gt;](xref:DotNext.Function`2) for static methods with return type
    * [Function&lt;T, TArgs, TResult&gt;](xref:DotNext.Function`3) for instance methods with return type
    * [Procedure&lt;TArgs&gt;](xref:DotNext.Procedure`1) for static methods without return type
    * [Procedure&lt;T, TArgs&gt;](xref:DotNext.Procedure`2) for instance methods without return type
...
If parameter in the method signature is declared as **ref** our **out** then field in such structure should be of type [Ref&lt;T&gt;](xref:DotNext.Reflection.Ref`1).
...
* [Record&lt;T&gt;](xref:DotNext.Record`1) represents [record class](https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/record)
